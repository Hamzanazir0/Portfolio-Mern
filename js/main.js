...
function loadMemberContent(memberID) {
  jQuery.ajax({
    url: jQuery('.ajax-member-content[data-id="' + memberID + '"]').attr(
      "href"
    ),
    type: "POST",
      ...
      setTimeout(function () {
        jQuery("#mcw-" + memberID).imagesLoaded(function () {
          ...
          jQuery("#team-holder .close-icon").on("click", function (e) {
            var memberReturnItemID = jQuery(this)
            ...
            setTimeout(function () {
              jQuery("html, body").animate(
                {
                  scrollTop:
                    jQuery("#team-member-" + memberReturnItemID).offset().top -
                    150,
                },
                400
              );
...
var portfolioItemContentLoadOnClick = function () {
  jQuery(".ajax-portfolio").on("click", function (e) {
    ...
    if (jQuery("#pcw-" + portfolioItemID).length) {
      //Check if is allready loaded
      jQuery("html, body").animate(
...
function loadPortfolioItemContent(portfolioItemID) {
  jQuery.ajax({
    url: jQuery('.ajax-portfolio[data-id="' + portfolioItemID + '"]').attr(
      "href"
    ),
    type: "POST",
    success: function (html) {
      var getPortfolioItemHtml = jQuery(html)
      ...
      if (!jQuery("#pcw-" + portfolioItemID + " .close-icon").length) {
        jQuery("#pcw-" + portfolioItemID).prepend(
      ...
      setTimeout(function () {
        jQuery("#pcw-" + portfolioItemID).imagesLoaded(function () {
          ...
          jQuery(".close-icon").on("click", function (e) {
            var portfolioReturnItemID = jQuery(this)
            ...
            setTimeout(function () {
              jQuery("html, body").animate(
                {
                  scrollTop:
                    jQuery("#p-item-" + portfolioReturnItemID).offset().top -
                    150,
                },
                400
              );
...
function splitSectionTitleFix() {
  ...
  jQuery(".section-title-holder.portfolio-title-fix-class")
    .stick_in_parent({
      offset_top: 64,
      parent: ".section-wrapper",
      spacer: ".sticky-spacer",
